...
2. Реализуем абстрактные методы
    ...
    По умолчанию предполагается, что объект хранит свое состояние в свойстве state и геттер и сеттер для него.
    В нашем случае у нас свойство passTicketStatus, поэтому перегружаем методы 
...
####<a name="transition"></a> Матрица переходов
